...
| 2026-06-10 | Phase 7.1 — `peek_one_chosen` redacted (cardId + handIndex → null) for non-peeker viewers                                                                                                 | Privacy leak: any room member could read another player's initial peeks via `getEventsSince`. Initial-peek picks are private per GAME_LOGIC; redaction now matches `peeked` semantics.                                                                                                                                                                                                      |
| 2026-06-10 | Phase 7.1 — `PlayerViewEntry.hasPeeked` public projection flag                                                                                                                            | The viewer's `knownCards` of an opponent says nothing about whether that opponent finished peeking (it tracks the _viewer's_ knowledge). A public boolean fixes the peek-overlay waiting count for online humans without leaking indices.                                                                                                                                                   |
| 2026-06-10 | Phase 7.1 — persistent `AnnouncementBanner` replaces toast double-duty; toasts reserved for errors                                                                                        | A memory game needs a "what just happened" line that survives the 1.8 s toast window. Planner toast cues now pin the announcement line (which re-animates per message); transient bottom toasts only carry rejected-move errors.                                                                                                                                                            |
| 2026-06-10 | Phase 7.1 — every silent event got text + motion: `match_failed` (reason toast), `power_activated` (actor focus), `peeked` (target spotlight), `swapped_blind`, `deck_reshuffled` (pulse) | "Every move is obvious" was the phase goal; the planner is the single choke point so all treatments are pure-function-tested in `flightPlanner.test.ts`.                                                                                                                                                                                                                                    |
| 2026-06-10 | Phase 7.1 — bot display names resolved via `t('botName.N')` in `displayName.ts`; `room.ts` exposes only `botIndex`                                                                        | The name literals in `room.ts` violated the i18n hard rule and duplicated the `botName.*` catalog keys.                                                                                                                                                                                                                                                                                     |
| 2026-06-10 | Phase 7.1 — removed dead store surface (`selection`/`dragInFlight`/`peekPicks`, unused selectors, unused tokens/i18n keys)                                                                | The tap-to-select model was superseded by the flow overlays in Phase 4.5; dead state attracts new misuse and inflates the public store contract.                                                                                                                                                                                                                                            |
